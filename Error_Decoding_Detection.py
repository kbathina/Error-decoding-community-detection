...
def F(a,x): 
    ...
    else: np.log((1.0 + atanh)/(1.0 - atanh))
...
if __name__ == "__main__":
	...
	iterations = sys.argv[2] # number of iterations (will output results from this many runs of the function)
	...
	if initial_condition != 'Random' or initial_condition != 'Regular':
    ...
    if type(iterations) != int:
        sys.exit("Number of runs is not an integer.")
	...
	for i in range(iterations): # for each iteration
	    ...
	    results[i] = run(graph,initial_condition) # store community labels in results dictionary

	with open("results.csv","w") as f: # open save file
	    wr = csv.writer(f) # open a writer
	    wr.writerows(results) # write results, each on its own line